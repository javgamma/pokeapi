...
const NewCard = ({pokemon, onClick, onRemove}) => {
  ...
  let cssClass = "";
      if(pokemon.id%2===0){
      cssClass='bg-violet-400 p-4 flex flex-col items-center rounded-xl w-[270px] min-h-[300px]'
      }else{
        cssClass='bg-blue-400 p-4 flex flex-col items-center rounded-xl w-[270px] min-h-[300px]'
      }
  ...
  return (
    <div className='flex justify-center'>
      <div className={cssClass}>
        ...
        <button className='flex' onClick={onClick}>🔄</button>
        <button className='flex'onClick={onRemove} >❌</button>
        </div>
      </div>
    </div>
  )
