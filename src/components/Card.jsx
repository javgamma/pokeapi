...
const Card = ({ creature, encounters }) => {
  ...
  return (
    ...
    <div className="flex flex-col gap-3">
      ...
      <p className='border rounded-lg p-2 box-border font-semibold'>id: {id}</p>
      <p className='border rounded-lg p-2 box-border font-semibold'>Base Experience: {base_experience}</p>
      <p className='border rounded-lg p-2 box-border font-semibold'>Height: {height}</p>
      <p className='border rounded-lg p-2 box-border font-semibold'>Weight: {weight}</p>
    </div>
      ...
      <div className=' flex flex-col w-[100%] items-center mb-4 p-5 justify-center'>
        ...
        <div className='flex flex-wrap gap-5 box-border'>
          {game_indices.map((game, index) => (
            <p className='border rounded-lg  box-border p-4' key={index}>{game.version.name}</p>
          ))}
        </div>
      </div>
      ...
      <div className='flex flex-col items-center h-[400px] justify-center '>
        ...
        <div className='flex flex-wrap gap-7 mt-2 justify-center text-white font-semibold'>
          {encounters.map((location, index) => (
            <p className='w-[190px] bg-violet-400 flex justify-center border rounded-lg p-2 items-center ' key={index}>{location.location_area.name}</p>
          ))}
        </div>
      </div>
      ...
      <div className='flex flex-col items-center mt-6'>
        ...
        <div className='flex w-[400px] h-[300px] items-center mt-4 mb-4 justify-center'>
          {images.map((image, index) => (
            <img key={index} src={image} alt={`sprite ${index}`} className="w-40 h-35 m-2" />
          ))}
        </div>
      </div>
      ...
      <div className='mb-10 flex flex-col items-center'>
        ...
        <div className="flex  gap-4">
          {stats.map((stat, index) => (
            <h1 key={index} className="bg-gray-200 rounded p-2 flex justify-center gap-3">
